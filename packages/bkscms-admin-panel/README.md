...
It will create two default users: ***superadmin*** & ***administrator***. Configure their credentials via `SUPERADMIN_*` and `ADMIN_*` environment variables (see AdminsTableSeeder.php for details). You must assign ***superadmins*** role to at least one user. **Important:** remove/disable the default users before rolling out to production for security reasons.  
